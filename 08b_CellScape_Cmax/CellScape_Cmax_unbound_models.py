...
def generate_moa_cmax(smiles):
    MOA =  filtered_molecule_dict.get(smiles, np.zeros(1893 , dtype=float))
    #print(MOA)
    cmax = [filtered_cmax_dict.get(smiles, 0)]
    #print(cmax)
...
for dataset in os.listdir(data_path):
    ...
    for activity in tqdm(activity_names, desc="Processing activities"):
        ...
        pd.DataFrame(results).T.to_csv('./CellScape_Cmax_unbound_model_results.csv')
...
results_df.to_csv('./CellScape_Cmax_unbound_model_results.csv', index=False)

# Concatenate and save held-out test set results
pd.concat(held_out_results).to_csv('./CellScape_Cmax_unbound_model_held_out_test_results.csv', index=False)
